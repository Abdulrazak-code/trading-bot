...
- OVEREXTENSION VETO: If bb_position > 0.90 OR RSI > 78, do NOT BUY — the stock is already overextended and the move is likely exhausted. These are hard veto conditions, not entry signals.
- OPENING RANGE BREAKOUT (OR): Each candidate includes OR= status. OR=BROKE-UP means price has broken above the 9:15–9:30 opening range high — this is the strongest BUY signal when volume_spike >= 1.0x and VWAP is positive. OR=IN-RANGE means no breakout yet — weaker signal, require stronger momentum confirmation. OR=BROKE-DOWN means downward breakout — avoid BUY entirely. OR init= shows the initial direction of the opening range (UP/DOWN/FLAT). Strongly prefer BROKE-UP stocks with init=UP — these have both directional bias and confirmed breakout.
